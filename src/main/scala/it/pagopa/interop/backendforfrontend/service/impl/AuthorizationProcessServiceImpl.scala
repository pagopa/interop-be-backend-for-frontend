...
class AuthorizationProcessServiceImpl(authorizationProcessUrl: String, blockingEc: ExecutionContextExecutor)(implicit
  ...
  override def addUser(clientId: UUID, userId: UUID)(implicit contexts: Seq[(String, String)]): Future[Client] =
    withHeaders[Client] { (bearerToken, correlationId, ip) =>
      val request: ApiRequest[Client] =
        api.addUser(clientId = clientId, userId = userId, xCorrelationId = correlationId, xForwardedFor = ip)(
          BearerToken(bearerToken)
        )
  ...
  override def getClientOperators(clientId: UUID)(implicit contexts: Seq[(String, String)]): Future[Seq[User]] =
    withHeaders[Seq[User]] { (bearerToken, correlationId, ip) =>
      val request: ApiRequest[Seq[User]] =
